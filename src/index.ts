 ...
 */
export function stringToASCIIArray(string: String): number[] {
 ...
 */
export function asciiArrayToBase2(ascii: number[]): string[] {
  return ascii
    .map((char) => {
 ...
 * of 8 "x" characters.
 */
export function zeroPad(array: string[]): string[] {
 ...
 * replaced with "0".
 */
export function chunkBinary(binaryNumber: string): string[] {
 ...
 */
export function base32ToBinary(base32: string): string[] {
  return base32.split("")
    .filter((char) => {
      return char !== "=";
    })
 ...
 */
export function binaryToASCII(binary: string[]): number[] {
  return binary.map((binaryNumber) => {
 ...
 */
export function asciiToString(ascii: number[]): string {
 ...
 */
export function fiveToEightBit(binary: string[]): string[] {
 ...
 */
export function bufferToBinaryArray(buffer: Buffer): string[] {
...
export function decode(base32: string): string {
  const fiveBitBinary = base32ToBinary(base32);
  const eightBitBinary = fiveToEightBit(fiveBitBinary);
  const ascii = binaryToASCII(eightBitBinary);
...
export function encode(data: string | Buffer): string {
  ...
  if (typeof data === "string") {
    const ascii = stringToASCIIArray(data);
    const binary = asciiArrayToBase2(ascii);
    binaryWithPadding = zeroPad(binary);
  } else if (isBuffer) {
    binaryWithPadding = zeroPad(bufferToBinaryArray(data));
  }
  const binaryChunks = chunkBinary(binaryWithPadding.join(""));
  return binaryArrayToBase32(binaryChunks);
}
